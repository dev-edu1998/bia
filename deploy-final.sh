...
# Script de Deploy ECS Final - Projeto BIA
# Versão: 1.0.0
...
TEMP_CURRENT=$(mktemp)
aws ecs describe-task-definition --task-definition $TASK_FAMILY --region $REGION --query 'taskDefinition' --output json > $TEMP_CURRENT

# Criar nova task definition com nova imagem
log_info "Criando nova task definition..."
TEMP_NEW=$(mktemp)
jq --arg image "$ECR_URI:$COMMIT_HASH" '.containerDefinitions[0].image = $image | del(.taskDefinitionArn, .revision, .status, .requiresAttributes, .placementConstraints, .compatibilities, .registeredAt, .registeredBy)' $TEMP_CURRENT > $TEMP_NEW

# Registrar nova task definition
log_info "Registrando nova task definition..."
TEMP_RESULT=$(mktemp)
aws ecs register-task-definition --region $REGION --cli-input-json file://$TEMP_NEW --output json > $TEMP_RESULT

# Extrair número da revisão
NEW_REVISION=$(jq -r '.taskDefinition.revision' $TEMP_RESULT)
...
rm -f $TEMP_CURRENT $TEMP_NEW $TEMP_RESULT
